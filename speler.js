...
if (member.user.bot === true) {
    ...
    bot = "<:user:424958082691629057> No";
            ...
            let embed = new Discord.RichEmbed()
                ...
                .addField("Bijnaam", `${member.nickname !== null ? `<:yes:425632265993846795> Nickname: ${member.nickname}` : "<:no:425632070036094986> None"}`, true)
                .addField("Bot", `${bot}`,inline, true)
                .addField("Status", `${status[member.user.presence.status]}`, inline, true)
                .addField("Speelt", `${member.user.presence.game ? `🎮 ${member.user.presence.game.name}` : "<:no:425632070036094986> Not playing"}`,inline, true)
                .addField("Roles", `${member.roles.filter(r => r.id !== message.guild.id).map(roles => `\`${roles.name}\``).join(" **|** ") || "<:no:425632070036094986> No Roles"}`, true)
                .addField("Discord Gejoined op", member.user.createdAt)
                .setFooter(`Informatie over ${member.user.username}`)
                .setTimestamp();
